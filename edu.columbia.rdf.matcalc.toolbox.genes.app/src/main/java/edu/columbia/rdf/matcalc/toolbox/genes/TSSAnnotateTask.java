...
public class TSSAnnotateTask extends SwingWorker<Void, Void> {
	...
	public TSSAnnotateTask(MainMatCalcWindow window,
			List<String> genomes, 
			int ext5p,
			int ext3p) {
		mWindow = window;
	...
	private DataFrame annotation() throws IOException, ParseException {
		...
		IterMap<GenomicRegion, IterMap<String, List<AnnotationGene>>> overlappingResults = 
				DefaultOrderedHashMap.create(new DefaultOrderedHashMapCreator<String, List<AnnotationGene>>(new ArrayListCreator<AnnotationGene>(100)));
		...
		for (int i = 0; i < model.getRows(); ++i) {
			...
			for (String genome : mGenomes) {
				FixedGapSearch<AnnotationGene> tssSearch = 
						AnnotationService.getInstance().getSearch(genome);

				List<AnnotationGene> results = 
						tssSearch.getFeatureSet(newRegion);

				for (AnnotationGene gene : results) {
					//System.err.println(region.getLocation() + ":" + gene.getSymbol() + " " + gene);
		...
		for (int i = 0; i < model.getRows(); ++i) {
			...
			for (String genome : mGenomes) {
				...
				List<AnnotationGene> results = 
						overlappingResults.get(region).get(genome);
