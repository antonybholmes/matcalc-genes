...
public class GeneAnnotationModule extends CalcModule implements ModernClickListener {
	...
	public void init(MainMatCalcWindow window) {
		...
		popup.addMenuItem(new ModernTwoLineMenuItem("Genes", 
				"Annotate regions for overlapping genes.", 
				ICON));
		popup.addMenuItem(new ModernTwoLineMenuItem("TSS",
				"Annotate for gene TSS within a region",
				ICON));
	...
	private void annotate() throws IOException {
		...
		AnnotateTask task = new AnnotateTask(mWindow,
				genomes,
				dialog.getShowOverlappingGenes(),
				dialog.getClosestList(),
				dialog.getExt5p(),
				dialog.getExt3p());
	...
	private void tssAnnotate() throws IOException {
		...
		TSSAnnotateTask task = new TSSAnnotateTask(mWindow,
				genomes,
				dialog.getExt5p(),
				dialog.getExt3p());
