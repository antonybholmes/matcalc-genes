...
public class AnnotateTask extends SwingWorker<Void, Void> {
	...
	public AnnotateTask(MainMatCalcWindow window,
			List<String> genomes, 
			boolean allSearch, 
			List<Integer> closestGenes,
			int ext5p,
			int ext3p) {
		mWindow = window;
		mGenomes = genomes;
	...
	private DataFrame annotation() throws IOException, ParseException {
		...
		for (String genome : mGenomes) {
			List<String> geneIdTypes = 
					AnnotationService.getInstance().getGeneIdTypes(genome);
		...
		for (int i = 0; i < model.getRows(); ++i) {
			...
			if (mShowOverlappingGenes) {
				for (String genome : mGenomes) {
					...
					List<AnnotationGene> results = 
							tssSearch.getFeatureSet(region);

					List<AnnotationGene> overlappingResults = 
							new ArrayList<AnnotationGene>(results.size());

					for (AnnotationGene gene : results) {
						System.err.println(region.getLocation() + ":" + gene.getSymbol() + " " + gene.getLocation());
					...
					for (AnnotationGene gene : overlappingResults) {
						...
						//System.err.println("gene " + gene.getSymbol() + " " + gene.getRefSeq() + " " + gene.getRegion().getStart() + " " + gene.getRegion().getEnd());
						...
						allClassifications.add(Join.onComma().values(CollectionUtils.reverse(CollectionUtils.sort(classifications))).toString());
					}
			...
			if (mClosestGenes.size() > 0) {
				...
				for (String genome : mGenomes) {
					...
					//System.err.println("closest " + closest + " " + c + " " + tssSearch.getName());

					// Find all closest genes between 1 and n
					List<List<AnnotationGene>> results = 
							tssSearch.getClosestFeatures(midRegion, maxClosest);

					for (int closest : mClosestGenes) {
						// Select the ones of interest
						c = annotateClosest(results.get(closest - 1), geneIdTypes, midPoint, i, c, matrix);
					}
				}
			}
		}
	...
	/**
	 * Create a semi colon separated list of gene ids from a set of
	 * genes.
	 * 
	 * @param name
	 * @param genes
	...
	private int createClosestHeader(String label,
		...
		matrix.setColumnName(c++, label + " Closest Region Relative To Gene (prom=-" + (mExt5p / 1000) + "/+" + (mExt3p / 1000) + "kb)");
	...
	private int annotateClosest(final List<AnnotationGene> results,
		...
		/*
		Map<String, AnnotationGene> entrezMap = new HashMap<String, AnnotationGene>();
		...
		if (results.size() > 0) {
			...
			//for (AnnotationGene gene : results) {
			tssDist = Math.min(tssDist, AnnotationGene.getTssMidDist(gene, midPoint));
			...
			if (withinBounds(gene, midPoint)) {
				...
				if (!inExon && 
						midPoint >= gene.getRegion().getStart() &&
						midPoint <= gene.getRegion().getEnd()) {
	...
	private int classify(AnnotationGene gene, 
		...
		if (withinBounds(gene, midPoint)) {
			...
			if (!inExon &&
					midPoint >= gene.getStart() &&
					midPoint <= gene.getEnd()) {
				classifications.add("intronic");
			}
		}
		//}
	...
	private boolean withinBounds(AnnotationGene gene, int midPoint) {
		if (gene.getStrand() == Strand.SENSE) {
			return midPoint >= gene.getStart() - mExt5p && 
					midPoint <= gene.getEnd();
		} else {
			return midPoint >= gene.getStart() && 
					midPoint <= gene.getEnd() + mExt5p;
		}
	}
}
